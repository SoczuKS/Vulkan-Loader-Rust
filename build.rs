...
fn main() {
    ...
    cmake::Config::new(".")
        .configure_arg(format!("-DUPDATE_DEPS=ON -C{}/deps/helper.cmake", out_dir))
        .build();
    ...
    let mut bindgen_builder = bindgen::Builder::default()
        .header(format!("{}/external/{}/64/Vulkan-Headers/build/install/include/vulkan/vulkan.h", env::var("CARGO_MANIFEST_DIR").unwrap(), env::var("PROFILE").unwrap()))
        .prepend_enum_name(false)
        .clang_arg(format!("-I{}/external/{}/64/Vulkan-Headers/build/install/include", env::var("CARGO_MANIFEST_DIR").unwrap(), env::var("PROFILE").unwrap()))
        .layout_tests(false)
        .allowlist_type("Vk.*")
